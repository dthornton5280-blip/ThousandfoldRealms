...
(async()=>{
  ...
  check(/^1\.6\.(?:1[3-9]|[2-9]\d)-dev$/.test(version.version),'version must identify v1.6.13-dev or later');
  ...
  check(cart.collisionFootprint.length===2,'scaled town cart must occupy its two-cell ground base');
  check(cart.collisionFootprint.some(cell=>cell.x===2&&cell.y===1),'cart collision must include the far wheel/base cell');
  check(cart.interactionFootprint.length===6,'cart interaction must cover the complete three-by-two visual footprint');
